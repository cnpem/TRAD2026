...
Dataset analisado: 16S_Hydrochoerus_hydrochaeris
...
R: Os arquivos que devem ser versionados são: scripts, metadados e arquivos de configuração. Os demais não precisam pois são arquivos brutos sem a necessidade de alteração.
...
R: Não é suficiente. Outros elementos serão necessários: versões de softwares; scripts bem documentados; parâmetros de qualidade e dependências.  
...
R: Observaria o efeito da qualidade média dos reads, o percentual de reads perdidos e tamanho médio dos reads. Para a documentação seriam necessários realizar um commit descrevendo a alteração, relatório comparando métricas antes e depois do trimming e atualização do README.
